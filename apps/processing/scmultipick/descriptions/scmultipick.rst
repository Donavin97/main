scmultipick applies threshold monitoring by searching for waveform anomalies in
...
.. note::
   ...
   Instead of detecting phase arrivals for source location, scmultipick
...
scmultipick detects phase onsets for generating :term:`picks <pick>`. Initally,
...
The second task of scmultipick is to calculate amplitudes of a given type for the
...
1. Generate standard publicIDs based on :confval:`publicIDPattern`. In addition to the
   available pattern tokens, scmultipick adds four new tokens: :code:`@net@`,
...
scmultipick usually runs in the background connected to a real-time data source
such as :ref:`Seedlink <seedlink>`. This is referred to as online mode. Another
option to run scmultipick is on offline mode with files.
...
* scmultipick reads all of its binding parameters and subscribes to stations
...
To run scmultipick in the background as a daemon module enable and start it ::

$ seiscomp enable scmultipick
$ seiscomp start scmultipick

For executing on the command line simply call it with appropriate options, e.g. ::

   $ seiscomp exec scmultipick -h
...
To tune scmultipick or to do playbacks it is helpful to run scmultipick not with
a real-time data source but on a defined data set, e.g. a multiplexed sorted miniSEED
volume. scmultipick will apply the same workflow as in online mode but the
acquisition of data records has to change. If the input data (file) has been
read, scmultipick will exit and furthermore it must not ask for a particular
...
.. code-block:: sh
   ...
   $ scmultipick --playback -I data.mseed
...
scmultipick will exit. The processing steps are similar to the online mode.
...
.. code-block:: sh
   ...
   $ scmultipick --playback -I data.mseed --ep -d [type]://[host]/[database] > picks.xml
