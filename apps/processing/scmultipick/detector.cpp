...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
void Detector::setAmplitudeFilter(const std::string &filter) {
	_amplFilterStr = filter;
	_amplFilter = nullptr;
}
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
void Detector::calculateMaxAmplitude(const Record *record, size_t i0, size_t i1, const DoubleArray &filteredData) {
	...
	// If an amplitude filter is configured, apply it to the original record data
	// for proper amplitude computation. This produces physical amplitudes (m/s)
	// rather than STALTA ratios when the detection filter includes STALTA.
	// This matches scautopick's approach where the AIC repicker computes
	// amplitudes from clean filtered data.
	if ( record && record->data() && !_amplFilterStr.empty() ) {
		// Lazy-initialize the amplitude filter
		if ( !_amplFilter ) {
			_amplFilter = Filter::Create(_amplFilterStr.c_str());
			if ( _amplFilter ) {
				try {
					_amplFilter->setSamplingFrequency(record->samplingFrequency());
					SEISCOMP_DEBUG("Amplitude filter created: %s @ %.1f Hz",
					               _amplFilterStr.c_str(), record->samplingFrequency());
				}
				catch ( std::exception &e ) {
					SEISCOMP_WARNING("Failed to init amplitude filter for fsamp = %.1f: %s",
		...
		// Apply amplitude filter to original record data
		if ( _amplFilter ) {
			DoubleArrayPtr rawData = (DoubleArray*)record->data()->copy(Array::DOUBLE);
			if ( rawData && rawData->size() > 0 ) {
				// Demean
				double mean = 0;
				double *d = rawData->typedData();
				...
				// Apply filter
				try {
					_amplFilter->apply(*rawData);

					// Compute max amplitude from filtered data
					size_t rawI1 = std::min((size_t)i1, (size_t)rawData->size());
					while ( i < rawI1 && _amplProc.processedSamples < _amplProc.neededSamples ) {
						double amp = fabs(d[i]);
					...
					SEISCOMP_WARNING("Amplitude filter apply failed: %s", e.what());
				}
			}
		}
	}

	// Fallback: use the detection filter's output (filteredData) directly
	// This is the original behavior, used when no amplitude filter is set
	// or when the amplitude filter approach failed.
