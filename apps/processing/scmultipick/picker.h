...
#ifndef SEISCOMP_APPLICATIONS_MULTIPICK_APP
#define SEISCOMP_APPLICATIONS_MULTIPICK_APP
...
class App : public Processing::Application {
	...
	private:
		...
		// Initialize a detector for P-phase detection
		bool initDetector(const std::string &streamID,
		                   const DataModel::WaveformStreamID &waveformID,
		                   const Record *rec);

		// Initialize a detector for S-phase detection on horizontal components
		bool initSDetector(const std::string &streamID,
		                   const DataModel::WaveformStreamID &waveformID,
		                   const Record *rec);
		...
		void emitRefinedPick(const Processing::Detector *pickProc,
		                     const Record *rec, const Core::Time& time,
		...
		std::string classifyPhase(const std::string& channelCode,
		                          const std::string& component,
		                          double snr,
		                          double distance = -1.0);

		// Determine phase type from channel code
		std::string phaseFromChannel(const std::string& channelCode);

		// Get horizontal component streams for a station
		bool getHorizontalComponents(const std::string& networkCode,
		                             const std::string& stationCode,
		                             const std::string& locationCode,
		                             const Core::Time& time,
		                             std::string& streamN,
	...
	private:
		...
		// Store last P picks per station to trigger S-phase search
		typedef std::map<std::string, DataModel::PickPtr> LastPPicksMap;

		StreamMap         _streams;
		Config            _config;
		PickMap           _lastPicks;
		LastPPicksMap     _lastPPicks;

		ProcMap           _runningStreamProcs;
		ProcReverseMap    _procLookup;

		StringSet         _streamIDs;
		StringSet         _sStreamIDs;  // S-phase detection streams
		...
		PhaseClassifier   _phaseClassifier;
};
