...
void App::printUsage() const {
	std::cout << "Usage:"  << std::endl << "  scpsloc [options]" << std::endl << std::endl
	     << "Advanced P/S phase locator for locating seismic events with improved depth resolution." << std::endl;

	Seiscomp::Client::Application::printUsage();

	std::cout << "Examples:" << std::endl;
	std::cout << "Real-time processing with informative debug output." << std::endl
	     << "  scpsloc --debug" << std::endl;
}
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
void App::createCommandLineDescription() {
	...
	// Phase clustering options
	commandline().addOption("Settings", "clustering-enable",
	                        "Enable DBSCAN-based phase clustering.",
	                        &_config.clusteringEnabled);
	commandline().addOption("Settings", "clustering-eps-time",
	                        "Time epsilon for clustering (seconds).",
	                        &_config.clusteringEpsTime);
	commandline().addOption("Settings", "clustering-eps-dist",
	                        "Distance epsilon for clustering (km).",
	                        &_config.clusteringEpsDist);
	commandline().addOption("Settings", "clustering-min-pts",
	                        "Minimum picks to form a cluster.",
	                        &_config.clusteringMinPts);
	commandline().addOption("Settings", "clustering-min-phases",
	                        "Minimum phases for a valid cluster.",
	                        &_config.clusteringMinPhases);
	commandline().addOption("Settings", "clustering-filter-picks",
	                        "Filter picks based on clustering results.",
	                        &_config.clusteringFilterPicks);
	commandline().addOption("Settings", "clustering-boost-score",
	                        "Boost score for picks in high-quality clusters.",
	                        &_config.clusteringBoostScore);
	commandline().addOption("Settings", "clustering-score-multiplier",
	                        "Score multiplier for clustered picks.",
	                        &_config.clusteringScoreMultiplier);
	...
	commandline().addOption("Settings", "require-amplitudes",
	                        "Require amplitudes for automatic picks (set false for AI pickers without amplitudes).",
	                        &_config.requireAmplitudes);
	commandline().addOption("Settings", "use-s-phases",
	                        "Enable S-phase usage in location.",
	                        &_config.useSPhases);
	commandline().addOption("Settings", "s-phase-weight",
	                        "Weight multiplier for S phases relative to P phases (default: 1.2).",
	                        &_config.sPhaseWeight);
	commandline().addOption("Settings", "min-sp-ratio",
	                        "Minimum S/P phase ratio for quality control (0.0 = no requirement).",
	                        &_config.minSPRatio);
	commandline().addOption("Settings", "max-sp-residual",
	                        "Maximum S-P time residual for association (seconds).",
	                        &_config.maxSPResidual);
	commandline().addOption("Settings", "min-score-s-phase",
	                        "Minimum origin score required for S phase association (default: 5.0).",
	                        &_config.minScoreSPhase);
	commandline().addOption("Settings", "min-score-other-s-phases",
	                        "Minimum origin score required for other S phase (ScS, SKS, etc.) association (default: 6.0).",
	                        &_config.minScoreOtherSPhases);
...
bool App::initConfiguration() {
	...
	// Phase clustering configuration
	try { _config.clusteringEnabled = configGetBool("autoloc.clustering.enable"); }
	catch (...) {}

	try { _config.clusteringEpsTime = configGetDouble("autoloc.clustering.epsTime"); }
	catch (...) {}

	try { _config.clusteringEpsDist = configGetDouble("autoloc.clustering.epsDist"); }
	catch (...) {}

	try { _config.clusteringMinPts = configGetInt("autoloc.clustering.minPts"); }
	catch (...) {}

	try { _config.clusteringTimeWeight = configGetDouble("autoloc.clustering.timeWeight"); }
	catch (...) {}

	try { _config.clusteringDistWeight = configGetDouble("autoloc.clustering.distWeight"); }
	catch (...) {}

	try { _config.clusteringUsePhaseType = configGetBool("autoloc.clustering.usePhaseType"); }
	catch (...) {}

	try { _config.clusteringUseTravelTime = configGetBool("autoloc.clustering.useTravelTime"); }
	catch (...) {}

	try { _config.clusteringMaxClusterTime = configGetDouble("autoloc.clustering.maxClusterTime"); }
	catch (...) {}

	try { _config.clusteringMinPhases = configGetInt("autoloc.clustering.minPhases"); }
	catch (...) {}

	try { _config.clusteringFilterPicks = configGetBool("autoloc.clustering.filterPicks"); }
	catch (...) {}

	try { _config.clusteringBoostScore = configGetBool("autoloc.clustering.boostScore"); }
	catch (...) {}

	try { _config.clusteringScoreThreshold = configGetDouble("autoloc.clustering.scoreThreshold"); }
	catch (...) {}

	try { _config.clusteringScoreMultiplier = configGetDouble("autoloc.clustering.scoreMultiplier"); }
	catch (...) {}

	try { _config.minPickSNR = configGetDouble("autoloc.minPickSNR"); }
	catch (...) {}

	try { _config.requireAmplitudes = configGetBool("autoloc.requireAmplitudes"); }
	catch (...) {}

	try { _config.useSPhases = configGetBool("autoloc.useSPhases"); }
	catch (...) {}

	try { _config.sPhaseWeight = configGetDouble("autoloc.sPhaseWeight"); }
	catch (...) {}

	try { _config.minSPRatio = configGetDouble("autoloc.minSPRatio"); }
	catch (...) {}

	try { _config.maxSPResidual = configGetDouble("autoloc.maxSPResidual"); }
	catch (...) {}
	...
	catch (...) { _gridConfigFile = Environment::Instance()->shareDir() + "/scpsloc/grid.conf"; }

	try { _config.staConfFile = Environment::Instance()->absolutePath(configGetString("autoloc.stationConfig")); }
	catch (...) { _config.staConfFile = Environment::Instance()->shareDir() + "/scpsloc/station.conf"; }
	...
	if ( ntp == "global" ) {
		_config.networkType = ::PSLoc::GlobalNetwork;
	}
	else if ( ntp == "regional" ) {
		_config.networkType = ::PSLoc::RegionalNetwork;
	}
	else if ( ntp == "local" ) {
		_config.networkType = ::PSLoc::LocalNetwork;
	}
	else {
...
bool App::initInventory() {
	if ( _stationLocationFile.empty() ) {
		...
		inventory = ::PSLoc::Utils::inventoryFromStationLocationFile(_stationLocationFile);
	}
...
bool App::feed(DataModel::Pick *scpick) {
	...
	::PSLoc::PickPtr pick = convertFromSC(scpick);
	if ( ! pick )
		return false;
...
bool App::feed(DataModel::Amplitude *scampl) {
	...
	SEISCOMP_DEBUG_S("Amplitude " + amplID + " type=" + atype + " for pick " + pickID);

	if ( atype != _amplTypeAbs && atype != _amplTypeSNR ) {
		SEISCOMP_DEBUG_S("Amplitude " + amplID + " rejected: type '" + atype + 
		                 "' doesn't match '" + _amplTypeAbs + "' or '" + _amplTypeSNR + "'");
		return false;
	}

	::PSLoc::Pick *pick = (::PSLoc::Pick *) Autoloc3::pick(pickID);
	if ( ! pick ) {
	...
	try {
		...
		if ( atype == _amplTypeSNR ) {
			...
			SEISCOMP_DEBUG_S("Amplitude " + amplID + " attached SNR=" + std::to_string(pick->snr) + " to pick " + pickID);
			
			// If both types are the same (e.g. both "snr"), also set the amplitude
			// This is needed because hasAmplitude() checks pick->amp, not pick->snr
			if ( _amplTypeSNR == _amplTypeAbs ) {
				pick->amp = scampl->amplitude().value();
				pick->per = (_amplTypeAbs == "mb") ? scampl->period().value() : 1;
				SEISCOMP_DEBUG_S("Amplitude " + amplID + " also set AMP=" + std::to_string(pick->amp) + " (same type as SNR)");
			}
		}
		if ( atype == _amplTypeAbs ) {
			pick->amp = scampl->amplitude().value();
			pick->per = (_amplTypeAbs == "mb") ? scampl->period().value() : 1;
			SEISCOMP_DEBUG_S("Amplitude " + amplID + " attached AMP=" + std::to_string(pick->amp) + " to pick " + pickID);
		}
	}
	catch ( ... ) {
		SEISCOMP_WARNING_S("Amplitude " + amplID + " failed to attach to pick " + pickID);
		return false;
	}

	SEISCOMP_DEBUG_S("Re-feeding pick " + pickID + " after amplitude attachment (amp=" + std::to_string(pick->amp) + ", snr=" + std::to_string(pick->snr) + ")");
	::PSLoc::Autoloc3::feed(pick);
	return true;
}
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
bool App::feed(DataModel::Origin *scorigin) {
	...
	::PSLoc::Origin *origin = convertFromSC(scorigin);
	if ( ! origin ) {
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
bool App::_report(const ::PSLoc::Origin *origin) {

	// Log object flow
	logObject(_outputOrgs, now());

	if ( _config.offline || _config.test ) {
		std::string reportStr = ::PSLoc::printDetailed(origin);
	...
	SEISCOMP_INFO_S(::PSLoc::printOrigin(origin, false));
