...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
static bool valid(const Pick *pick, bool requireAmplitudes=true)
{
	...
	// S-phase picks are valuable for location even without amplitudes
	bool isSPhase = (pick->label.find("-A-S-") != std::string::npos ||
	                 pick->label.find("-S-") != std::string::npos);

	// If amplitudes are not required (e.g., AI pickers), skip SNR and amplitude checks
	// This allows picks without amplitude information to be processed
	if (requireAmplitudes) {
		// SNR check - skip for S phases
		if ( !isSPhase && (pick->snr <= 0 || pick->snr > 1.0E7) ) {
			if (pick->snr > 1.0E7)
		...
		// Amplitude check - skip for S phases
		if ( !isSPhase && ! hasAmplitude(pick))
			return false;
	}
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
// Validate S-phase pick against S-P time consistency for same station.
// Returns true if either no P pick exists at that station (can't validate)
// or the observed S-P time matches the theoretical value within tolerance.
static bool validateSPConsistency(const Origin* origin, const Pick* sPick,
                                   const Station* station, double toleranceSec)
{
	// Find P-phase arrival at same station
	const Pick* pPick = nullptr;
	for (const auto& arr : origin->arrivals) {
		if (arr.excluded)
			continue;
		if (arr.pick->station() != station)
			continue;
		if (arr.phase == "P" || arr.phase == "PKP" || arr.phase == "Pg" || arr.phase == "Pn") {
			pPick = arr.pick.get();
			break;
		}
	}
	if (!pPick)
		return true;  // No P at this station yet, can't validate

	// Compute observed and theoretical S-P times
	TravelTime ttP, ttS;
	double delta, az, baz;
	delazi(&origin->hypocenter, station, delta, az, baz);

	if (!travelTimeP(origin->hypocenter.lat, origin->hypocenter.lon,
	                 origin->hypocenter.dep, station->lat, station->lon, 0, delta, ttP))
		return true;
	if (!travelTimeS(origin->hypocenter.lat, origin->hypocenter.lon,
	                 origin->hypocenter.dep, station->lat, station->lon, 0, delta, ttS))
		return true;

	double observedSP = sPick->time - pPick->time;
	double theoreticalSP = ttS.time - ttP.time;
	double diff = std::fabs(observedSP - theoreticalSP);

	SEISCOMP_DEBUG("S-P validation: station %s obs=%.2f th=%.2f diff=%.2f (tol=%.1f)",
	               station->code.c_str(), observedSP, theoreticalSP, diff, toleranceSec);
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
// Compute adaptive S-phase residual tolerance based on epicentral distance.
// Nearby stations get tighter tolerance; distant stations get more slack.
static double adaptiveSResidualTolerance(double deltaDeg)
{
	// Base tolerance: 3 s at 0 deg, increasing to 12 s at 180 deg
	return 3.0 + 9.0 * (deltaDeg / 180.0);
}
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
bool Autoloc3::init()
{
	_relocator.setSeiscompConfig(_config.scconfig);
        if ( ! _relocator.init()) {
		SEISCOMP_ERROR("PSLoc::init(): Failed to initialize relocator");
                return false;
	}
...
bool Autoloc3::report()
{
	...
	// Improvement 2: Merge duplicate origins before flushing
	// This consolidates picks from nearby origins and reduces false events
	_mergeDuplicateOrigins();
...
bool Autoloc3::_store(const Pick *pick)
{
	...
	// If the pick already exists, we need to update it (e.g., when re-feeding after amplitude attachment)
	const Pick *existingPick = Autoloc3::pick(pick->id);
	if ( existingPick ) {
		// Pick exists - update the stored version with new amplitude values
		const_cast<Pick*>(existingPick)->amp = pick->amp;
		const_cast<Pick*>(existingPick)->snr = pick->snr;
		const_cast<Pick*>(existingPick)->per = pick->per;
		SEISCOMP_DEBUG("Updated existing pick %s (amp=%g, snr=%g)", pick->id.c_str(), pick->amp, pick->snr);
	}
	else {
		// New pick - store it
...
bool Autoloc3::feed(const Pick *pick)
{
	...
	// EXCEPTION: S-phase picks can be processed without amplitudes
	// because they're valuable for location even without amplitude info
	// EXCEPTION: If requireAmplitudes is false, all picks can be processed without amplitudes
	if ( automatic(pick) ) {
		bool isSPhase = (pick->label.find("-A-S-") != std::string::npos ||
		                 pick->label.find("-S-") != std::string::npos);
		bool needAmplitude = _config.requireAmplitudes && !isSPhase;

		if ( needAmplitude && ! hasAmplitude(pick)) {
			if (isnew)
				SEISCOMP_DEBUG("process pick %s waiting for amplitude (amp=%g, snr=%g)",
					       pick->label.c_str(), pick->amp, pick->snr);
			return false;
		}
		else {
			if (!hasAmplitude(pick)) {
				if (isSPhase)
					SEISCOMP_INFO("process pick %s (S-phase, no amplitude)", pick->label.c_str());
				else
					SEISCOMP_INFO("process pick %s (no amplitude, requireAmplitudes=false)", pick->label.c_str());
			}
			else
				SEISCOMP_INFO("process pick %s (amp=%g, snr=%g)",
					      pick->label.c_str(), pick->amp, pick->snr);
		}
	}
...
bool Autoloc3::feed(Origin *origin)
{
	...
	if (found) {
		...
		// Explicitly copy fields to avoid deprecated implicit assignment operator
		found->hypocenter = manualOrigin->hypocenter;
		found->time = manualOrigin->time;
		found->timeerr = manualOrigin->timeerr;
		found->depthType = manualOrigin->depthType;
		found->quality = manualOrigin->quality;
		found->error = manualOrigin->error;
		found->imported = manualOrigin->imported;
		found->preliminary = manualOrigin->preliminary;
		found->publicID = manualOrigin->publicID;
		found->methodID = manualOrigin->methodID;
		found->earthModelID = manualOrigin->earthModelID;
		found->processingStatus = manualOrigin->processingStatus;
		found->locationStatus = manualOrigin->locationStatus;
		found->score = manualOrigin->score;
		found->scorigin = manualOrigin->scorigin;
...
bool Autoloc3::_tooLowSNR(const Pick *pick) const
{
	...
	// If amplitudes are not required, skip SNR check
	// Picks from AI pickers without amplitudes will have SNR=0
	if (!_config.requireAmplitudes)
		return false;
...
bool Autoloc3::_tooManyRecentPicks(const Pick *newPick) const
{
	...
	// S-phase picks are valuable and should not be rejected for lacking SNR
	// Use phase detection from pick label
	bool labelHasS = (newPick->label.find("-A-S-") != std::string::npos ||
	                  newPick->label.find("-S-") != std::string::npos);

	if (newPick->snr <= 0.) {
		if (labelHasS) {
			SEISCOMP_DEBUG("_tooManyRecentPicks: S-phase pick without snr amplitude: %s -> allowing", newPick->label.c_str());
		}
		else {
			// If requireAmplitudes is false, allow picks without SNR
			if (!_config.requireAmplitudes) {
				SEISCOMP_DEBUG("_tooManyRecentPicks: pick without snr but requireAmplitudes=false: %s -> allowing", newPick->label.c_str());
			}
			else {
	...
	// Skip SNR-based rejection for S-phase picks since they don't have SNR values
	if (!labelHasS) {
		...
		SEISCOMP_DEBUG("_tooManyRecentPicks: S-phase pick %s allowed (skipping SNR check)", newPick->label.c_str());
	}
...
{
	// S-phase picks naturally follow P-phase picks from the same event
	// They should not be rejected for following bigger picks
	bool isSPhase = (newPick->label.find("-A-S-") != std::string::npos || 
	                 newPick->label.find("-S-") != std::string::npos);
	...
	for (auto& item : pickPool) {
		...
		// S-phase picks are allowed to follow XXL picks (they're from the same event)
		if (isSPhase) {
			SEISCOMP_DEBUG("S-phase pick %s allowed (follows XXL pick %s, but S phases naturally follow P)", 
			              newPick->label.c_str(), pick->label.c_str());
			continue;
		}
...
OriginPtr Autoloc3::_tryAssociate(const Pick *pick)
{
	...
	std::set<OriginID> associatedOriginIds;
	for (const Association& asso : associations) {
		OriginPtr associatedOrigin = new Origin(*asso.origin);

		// this is the main criteria
		if (asso.affinity < _config.minPickAffinity) {
			SEISCOMP_DEBUG("Association skipped: affinity %.2f < min %.2f", asso.affinity, _config.minPickAffinity);
			continue;
		}
		...
		// Skip if we already successfully associated this pick with this origin
		if (associatedOriginIds.count(asso.origin->id)) {
			SEISCOMP_DEBUG("Skipping duplicate association to origin %ld", asso.origin->id);
			continue;
		}

		SEISCOMP_DEBUG("Trying association: origin %ld, phase '%s' (len=%zu), affinity %.2f",
		               asso.origin->id, asso.phase.c_str(), asso.phase.size(), asso.affinity);

		if (asso.phase == "P" || asso.phase == "PKP") {
			SEISCOMP_INFO("PHASE MATCH: P/PKP for origin %ld, pick %s", asso.origin->id, pick->label.c_str());
			SEISCOMP_DEBUG("_associate called for origin %ld phase %s", asso.origin->id, asso.phase.c_str());
			bool success = _associate(associatedOrigin.get(), pick, asso.phase);
		...
		else if (asso.phase == "S" || asso.phase == "SKS") {
			// NEW: Handle S phases - associate them like P phases for improved location
			SEISCOMP_INFO("PHASE MATCH: S/SKS for origin %ld, pick %s", asso.origin->id, pick->label.c_str());
			SEISCOMP_DEBUG("_associate called for origin %ld phase %s", asso.origin->id, asso.phase.c_str());
			bool success = _associate(associatedOrigin.get(), pick, asso.phase);
			std::string oneliner = printOneliner(associatedOrigin.get())+"  ph="+asso.phase;
		...
		// Track that we successfully associated this pick with this origin
		// to prevent duplicate associations from subsequent loop iterations
		associatedOriginIds.insert(asso.origin->id);

		// Update the original origin in _origins with the new pick
		// This is important for preliminary origins to accumulate picks
		if (asso.origin->preliminary) {
			// Find the origin in _origins by ID and update it
			for (auto& orig : _origins) {
				if (orig->id == asso.origin->id) {
					orig->updateFrom(associatedOrigin.get());
					SEISCOMP_DEBUG("Updated preliminary origin %ld with new pick (total arrivals: %zu, defining: %zu)",
					               orig->id, orig->arrivals.size(), orig->definingPhaseCount());

					// If we now have enough total arrivals, try to relocate and promote picks
					if (orig->arrivals.size() >= (size_t)_config.minPhaseCount) {
						SEISCOMP_DEBUG("Trying to relocate preliminary origin %ld with %zu arrivals",
						               orig->id, orig->arrivals.size());

						// Convert temporarily excluded picks to NotExcluded for relocation attempt
						for (auto& arr : orig->arrivals) {
							if (arr.excluded == Arrival::TemporarilyExcluded) {
								arr.excluded = Arrival::NotExcluded;
							}
						}

						// Try relocation with fixed depth first
						_relocator.setFixedDepth(_config.defaultDepth);
						_relocator.useFixedDepth(true);
						OriginPtr relo = _relocator.relocate(orig.get());

						if (relo && relo->definingPhaseCount() >= (size_t)_config.minPhaseCount) {
							// Relocation succeeded with enough phases - promote origin
							orig->updateFrom(relo.get());
							orig->preliminary = false;
							SEISCOMP_INFO("Promoted preliminary origin %ld to full origin with %zu defining phases",
							              orig->id, orig->definingPhaseCount());

							// Try to associate S-phase picks to improve location
							_tryAssociateSPicks(orig.get());
						}
						else {
							// Relocation didn't succeed well enough - revert picks to TemporarilyExcluded
							// Keep only the original defining picks as NotExcluded
							size_t definingCount = 0;
							for (auto& arr : orig->arrivals) {
								if (definingCount < (size_t)_config.minPhaseCount) {
									arr.excluded = Arrival::NotExcluded;
									definingCount++;
								}
								else {
...
OriginPtr Autoloc3::_tryNucleate(const Pick *pick) {
	...
	// Improvement 1: For preliminary origins (single-station), compute centroid
	// of recent picks and relocate origin there for better initial placement
	if (newOrigin->preliminary || newOrigin->definingPhaseCount() < 2) {
		double sumLat = 0, sumLon = 0;
		int pickCount = 0;

		// Use recent picks from the last 30 seconds to compute centroid
		Time cutoffTime = pick->time - Time(30);

		for (const auto& pair : pickPool) {
			const Pick* p = pair.second.get();
			if (p->time < cutoffTime || p->time > pick->time + Time(5)) continue;
			if (!p->station()) continue;

			sumLat += p->station()->lat;
			sumLon += p->station()->lon;
			pickCount++;
		}

		if (pickCount >= 2) {
			double centroidLat = sumLat / pickCount;
			double centroidLon = sumLon / pickCount;

			SEISCOMP_DEBUG("Relocating preliminary origin from station (%.3f, %.3f) to pick centroid (%.3f, %.3f) from %d picks",
			               newOrigin->hypocenter.lat, newOrigin->hypocenter.lon,
			               centroidLat, centroidLon, pickCount);

			// Create a temporary copy with centroid location
			OriginPtr centroidOrigin = new Origin(centroidLat, centroidLon,
			                                      newOrigin->hypocenter.dep,
			                                      newOrigin->time);
			centroidOrigin->depthType = newOrigin->depthType;
			centroidOrigin->preliminary = newOrigin->preliminary;

			// Copy arrivals to centroid origin
			for (const auto& arr : newOrigin->arrivals) {
				centroidOrigin->arrivals.push_back(arr);
			}

			// Try to relocate with centroid position
			_relocator.setFixedDepth(_config.defaultDepth);
			_relocator.useFixedDepth(true);
			OriginPtr relo = _relocator.relocate(centroidOrigin.get());

			if (relo) {
				newOrigin->updateFrom(relo.get());
				SEISCOMP_DEBUG("Successfully relocated preliminary origin to centroid position");
			}
			else {
				// If relocation fails, at least use the centroid position
				newOrigin->hypocenter.lat = centroidLat;
				newOrigin->hypocenter.lon = centroidLon;
				SEISCOMP_DEBUG("Relocation failed, using centroid position as fallback");
			}
		}
	}
	...
	if ( ! bestEquivalentOrigin ) {
		if ( _passedFilter(newOrigin.get()) ) {
			...
			// Following scanloc approach: store as preliminary origin even if it
			// doesn't pass the filter yet. It may accumulate more picks later.
			// Try to associate additional picks first to improve it.
			_tryAssociateAdditionalPicks(newOrigin.get());

			// Try relocating with additional picks
			OriginPtr relo = _relocator.relocate(newOrigin.get());
			if ( relo ) {
				newOrigin->updateFrom(relo.get());
				if ( _passedFilter(newOrigin.get()) ) {
					return newOrigin;
			...
			// Still doesn't pass filter, but keep as preliminary
			newOrigin->preliminary = true;
			newOrigin->id = _newOriginID();
			newOrigin->arrivals.sort();
			SEISCOMP_INFO("Storing preliminary origin %ld with %zu arrivals (doesn't pass filter yet)",
			              newOrigin->id, newOrigin->arrivals.size());
			_store(newOrigin.get());
			return newOrigin;
		}
	}
...
{
	// process a pick
	if ( ! valid(pick, _config.requireAmplitudes) ) {
		SEISCOMP_DEBUG("invalid pick %s", pick->label.c_str());
		return false;
	}

	if ( automatic(pick) && _tooLowSNR(pick) ) {
		// Check if this is an S-phase pick - if so, allow it even without SNR
		bool isSPhase = (pick->label.find("-A-S-") != std::string::npos || 
		                 pick->label.find("-S-") != std::string::npos);
		if (!isSPhase)
			return false;
		else
			SEISCOMP_DEBUG("S-phase pick %s proceeding without SNR", pick->label.c_str());
	}
	...
	// Check if this is an S-phase pick for logging
	bool isSPhase = (pick->label.find("-A-S-") != std::string::npos || 
	                 pick->label.find("-S-") != std::string::npos);

	if ( _followsBiggerPick(pick) ) {
		if (isSPhase) SEISCOMP_DEBUG("S-phase pick %s rejected by _followsBiggerPick", pick->label.c_str());
		return false;
	}

	if ( _perhapsPdiff(pick) ) {
		if (isSPhase) SEISCOMP_DEBUG("S-phase pick %s rejected by _perhapsPdiff", pick->label.c_str());
		return false;
	}

	// Run phase clustering if enabled.
	// Clustering is used for supplementary quality assessment (score boosting),
	// NOT for hard-filtering picks. Picks must always be allowed to reach the
	// nucleator/associator so that new events can be detected from the first
	// few arrivals. Hard-filtering by cluster membership prevents nucleation
	// because the picks needed to form a cluster get rejected before the
	// cluster can exist.
	if ( _config.clusteringEnabled ) {
		_runClustering();
	}
	...
	if (isSPhase) SEISCOMP_INFO("S-phase pick %s attempting association", pick->label.c_str());
	...
	if ( origin ) {
		...
		// Try to associate additional picks to improve the origin
		_tryAssociateAdditionalPicks(origin.get());

		// Try to rework if we have enough phases
		if ( origin->definingPhaseCount() >= _config.minPhaseCount ) {
			_rework(origin.get());
		}

		if ( _passedFilter(origin.get()) ) {
			// Origin passes filter - promote from preliminary
			origin->preliminary = false;
			_store(origin.get());

			// Try to associate S-phase picks to improve location
			_tryAssociateSPicks(origin.get());

			return true;
		}
		else {
			// Origin doesn't pass filter yet - keep as preliminary
			// Don't set origin to nullptr, we still want to keep it
			origin->preliminary = true;
			SEISCOMP_DEBUG("Origin %ld associated pick but doesn't pass filter yet (%zu phases)",
			               origin->id, origin->definingPhaseCount());
			_store(origin.get());
		}
	}
	...
	// Don't try to nucleate origins from S-phase picks alone
	// S-phases should associate to existing P-phase originated origins
	// This follows the scanloc approach: P-phases create the origin, S-phases associate to it
	if ( isSPhase ) {
		SEISCOMP_DEBUG("S-phase pick %s not attempting nucleation (waiting for P-phase origin)",
		               pick->label.c_str());
		// Store the pick but don't create an origin from it
		_store(pick);
		return false;
	}
	...
	if ( origin ) {
		// Origin was created (may be preliminary)
		// Try to associate additional picks to improve it
		_tryAssociateAdditionalPicks(origin.get());

		// Only try _rework if we have enough phases
		// For preliminary origins with few picks, skip rework and just store
		if ( origin->definingPhaseCount() >= _config.minPhaseCount ) {
			// Try to rework and validate the origin
			if ( _rework(origin.get()) ) {
				if ( _passedFilter(origin.get()) ) {
					// Origin passes filter - promote from preliminary if needed
					origin->preliminary = false;
					_store(origin.get());

					// Try to associate S-phase picks to improve location (scanloc approach)
					_tryAssociateSPicks(origin.get());

					return true;
				}
				else {
					// Origin doesn't pass filter but keep as preliminary
					origin->preliminary = true;
					SEISCOMP_INFO("Origin %ld stored as preliminary (doesn't pass filter yet, %zu phases)",
					              origin->id, origin->definingPhaseCount());
					_store(origin.get());
				}
			}
			else {
				// Rework failed - still keep as preliminary
				origin->preliminary = true;
				SEISCOMP_INFO("Origin %ld stored as preliminary (rework failed, %zu phases)",
				              origin->id, origin->definingPhaseCount());
				_store(origin.get());
			}
		}
		else {
			// Not enough phases yet - store as preliminary
			origin->preliminary = true;
			SEISCOMP_DEBUG("Origin %ld stored as preliminary (only %zu phases, need %d)",
			               origin->id, origin->definingPhaseCount(), _config.minPhaseCount);
			_store(origin.get());
		}
	}
	...
	// Note: XXL should only be attempted for P-phase picks

	if (_config.xxlEnabled && !isSPhase) {
...
bool Autoloc3::_publishable(const Origin *origin) const
{
	...
	// Enforce hard minimum score. Origins below this threshold are kept
	// internally but never published/sent.
	...
	// Optional S/P ratio quality control
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
void Autoloc3::_mergeDuplicateOrigins()
{
	// Improvement 2: Merge duplicate origins that are close in space and time
	// This reduces false event count and consolidates picks into stronger origins

	// Configuration for merging
	const double maxSpatialDistanceKm = 50.0;  // Maximum distance between origins to consider merging
	const double maxTimeDifference = 10.0;     // Maximum time difference (seconds)
	const int minSharedPicks = 1;              // Minimum number of shared picks to trigger merge

	std::set<OriginID> mergedOrigins;  // Track which origins have been merged

	for (size_t i = 0; i < _origins.size(); ++i) {
		Origin* origin1 = _origins[i].get();

		// Skip already merged origins
		if (mergedOrigins.count(origin1->id)) continue;

		for (size_t j = i + 1; j < _origins.size(); ++j) {
			Origin* origin2 = _origins[j].get();

			// Skip already merged origins
			if (mergedOrigins.count(origin2->id)) continue;

			// Check time difference
			double dt = std::fabs(origin1->time - origin2->time);
			if (dt > maxTimeDifference) continue;

			// Check spatial distance
			double delta = 0, az = 0, baz = 0;
			delazi(origin1->hypocenter.lat, origin1->hypocenter.lon,
			       origin2->hypocenter.lat, origin2->hypocenter.lon,
			       delta, az, baz);
			double distanceKm = delta * 111.195;  // Convert degrees to km
			if (distanceKm > maxSpatialDistanceKm) continue;

			// Check for shared picks
			std::set<const Pick*> picks1, picks2;
			for (const auto& arr : origin1->arrivals) {
				if (arr.pick && !arr.excluded) picks1.insert(arr.pick.get());
			}
			for (const auto& arr : origin2->arrivals) {
				if (arr.pick && !arr.excluded) picks2.insert(arr.pick.get());
			}

			int sharedCount = 0;
			for (const auto* pick : picks1) {
				if (picks2.count(pick)) sharedCount++;
			}

			// If origins share picks and are close, merge them
			if (sharedCount >= minSharedPicks || (origin1->preliminary && origin2->preliminary)) {
				// Determine which origin to keep (prefer higher score or more picks)
				Origin* keep = origin1;
				Origin* merge = origin2;

				double score1 = origin1->score;
				double score2 = origin2->score;

				// Prefer the origin with better score, or more picks if scores are similar
				if (score2 > score1 * 1.1 || (std::fabs(score2 - score1) < 0.1 && origin2->arrivals.size() > origin1->arrivals.size())) {
					keep = origin2;
					merge = origin1;
				}

				SEISCOMP_INFO("Merging origin %ld into %ld (distance=%.1f km, dt=%.1f s, shared=%d, scores: %.1f vs %.1f)",
				              merge->id, keep->id, distanceKm, dt, sharedCount, origin1->score, origin2->score);

				// Merge picks from merge origin into keep origin
				int picksMerged = 0;
				for (const auto& arr : merge->arrivals) {
					if (!arr.pick) continue;

					// Check if pick already exists in keep origin
					bool exists = false;
					for (const auto& existing : keep->arrivals) {
						if (existing.pick && existing.pick->id == arr.pick->id) {
							exists = true;
							break;
						}
					}
					...
					if (!exists) {
						// Add pick to keep origin
						Arrival newArr = arr;
						newArr.origin = keep;
						keep->arrivals.push_back(newArr);
						picksMerged++;
					}
				}

				// Mark merge origin as merged
				mergedOrigins.insert(merge->id);

				SEISCOMP_DEBUG("Merged %d picks, total arrivals in origin %ld: %zu",
				               picksMerged, keep->id, keep->arrivals.size());

				// Relocate the merged origin
				_relocator.useFixedDepth(false);
				OriginPtr relo = _relocator.relocate(keep);
				if (relo) {
					keep->updateFrom(relo.get());
					_updateScore(keep);
					SEISCOMP_DEBUG("After merge relocation: origin %ld score=%.1f, phases=%zu",
					               keep->id, keep->score, keep->definingPhaseCount());
				}
			}
		}
	}

	// Remove merged origins from _origins
	if (!mergedOrigins.empty()) {
		_origins.erase(
			std::remove_if(_origins.begin(), _origins.end(),
			               [&mergedOrigins](const OriginPtr& orig) {
				               return mergedOrigins.count(orig->id) > 0;
			               }),
			_origins.end()
		);

		SEISCOMP_INFO("Removed %zu merged origins, %zu origins remaining",
		              mergedOrigins.size(), _origins.size());
	}
}
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
{
	SEISCOMP_DEBUG("_associate: origin %ld phase %s pick %s", origin->id, phase.c_str(), pick->label.c_str());

	// first crude check
	if ( ! mightBeAssociated(pick, origin)) {
		SEISCOMP_DEBUG("_associate: mightBeAssociated returned false");
		return false;
	}
	...
	// Handle depth phases and other specific phases
	else if (isDepthPhase(phase) || isRegionalPhase(phase)) {
		// Use travel time table to find the specific phase
		static Seiscomp::TravelTimeTable ttt;
		Seiscomp::TravelTimeList *ttlist {nullptr};
		try {
			ttlist = ttt.compute(origin->hypocenter.lat, origin->hypocenter.lon, std::max(origin->hypocenter.dep, 0.01),
			                     station->lat, station->lon, 0);
		}
		catch ( std::out_of_range & ) {
			return false;
		...
		bool found = false;
		for (const auto& t : *ttlist) {
			if (t.phase == phase || t.phase.substr(0, phase.size()) == phase) {
				tt = t;
				found = true;
				SEISCOMP_DEBUG("Found phase %s travel time=%.2f", phase.c_str(), tt.time);
				break;
			}
		}
		delete ttlist;
		if (!found) {
			SEISCOMP_WARNING("Phase %s not found in travel time table", phase.c_str());
			return false;
		}
	}
	else {
		SEISCOMP_WARNING_S("_associate got unrecognized phase: " + phase + " - ignored");
		return false;
	}
	...
	// Debug logging for S phases
	if (phase == "S" || phase == "SKS") {
		SEISCOMP_INFO("S-phase %s: pick_time=%.3f origin_time=%.3f tt=%.3f residual=%.3f",
		             pick->label.c_str(), pick->time, origin->time, tt.time, residual);
	}

	// For preliminary origins with few picks, be more lenient with residual checks
	// The origin time and location are just initial guesses that will be refined
	bool isPreliminary = origin->preliminary && origin->definingPhaseCount() < 3;
	double residualMultiplier = isPreliminary ? 3.0 : 1.0;  // 3x more lenient for preliminary origins
	SEISCOMP_DEBUG("_associate: preliminary=%d phaseCount=%zu multiplier=%.1f",
	               isPreliminary ? 1 : 0, origin->definingPhaseCount(), residualMultiplier);

	// S phases get more generous residual tolerance
	if (phase == "S" || phase == "SKS") {
		if ( ! _residualOK(arr, 1.5 * residualMultiplier, 2.0 * residualMultiplier)) {
			SEISCOMP_DEBUG("_associate: S-phase residual check failed (residual=%.2f)", arr.residual);
			return false;
		}
	}
	else {
		if ( ! _residualOK(arr, 0.9 * residualMultiplier, 1.3 * residualMultiplier)) {
			SEISCOMP_DEBUG("_associate: P-phase residual check failed (residual=%.2f)", arr.residual);
			return false;
		}
	}
	arr.excluded = Arrival::NotExcluded;
	...
	// S phases are used for improved location quality
	// Regional phases are valuable for local/regional networks
	if (phase == "P" || phase == "PKP" || phase == "Pg" || phase == "Pn" || phase == "Pb") {
	...
	else if (phase == "S" || phase == "SKS" || phase == "Sg" || phase == "Sn" || phase == "Sb") {
		// S phases are valuable for location, use them unless too distant
		if (delta > 105 && copy->definingPhaseCount(0,105.) > _config.minStaCountIgnorePKP * 2)
			arr.excluded = Arrival::UnusedPhase;
		else
			arr.excluded = Arrival::NotExcluded;
	}
	else if (isRegionalPhase(phase)) {
		// Regional phases (ScP, ScS, PcP) are valuable
		arr.excluded = Arrival::NotExcluded;
	}
	else if (isDepthPhase(phase)) {
		// Depth phases very valuable for depth resolution
		arr.excluded = Arrival::NotExcluded;
	}
	else {
		arr.excluded = Arrival::UnusedPhase;
...
//			else if (origin->depthType == Origin::DepthManuallyFixed || origin->depthType == Origin::DepthPhases) {
			...
			if ( ! relo ) {
				// For preliminary origins with few picks, allow association even if relocation fails
				// The origin will be relocated properly once more picks are associated
				if (origin->preliminary && origin->definingPhaseCount() < 3) {
					SEISCOMP_DEBUG("_associate: relocation failed for preliminary origin %ld, keeping pick loosely associated",
					               origin->id);
					arr.distance = delta;
					arr.azimuth = az;
					arr.excluded = Arrival::TemporarilyExcluded;  // Mark as temp excluded until more picks
					origin->add(arr);
					return true;
				}
			...
			// Be more lenient with S phases - they're valuable even if they don't immediately improve score
			bool isSPhase = (arr.phase == "S" || arr.phase == "SKS");
			double rms_tolerance = isSPhase ? 5.0 : 3.0;  // S phases get more RMS tolerance
			
			if (score_after_relocation < original_score ||
			    rms_after_relocation > original_rms + rms_tolerance/sqrt(10.+copy->arrivals.size())) {
				// no improvement
				...
				// For S phases, keep them associated even if they don't improve score
				// They're still valuable for constraining the hypocenter
				if (isSPhase) {
					SEISCOMP_DEBUG("S phase %s didn't improve score but keeping it associated", pick->label.c_str());
					arr.excluded = Arrival::NotExcluded;  // Keep S phase in location
				}
				else {
		...
		if (relo) {
			...
			// Relocation failed - but for S phases, keep them associated anyway
			// They're still valuable for constraining the hypocenter even with larger residuals
			bool isSPhase = (arr.phase == "S" || arr.phase == "SKS");
			
			copy = new Origin(*origin);
			if (arr.excluded != Arrival::UnusedPhase) {
				if (isSPhase) {
					// Keep S phase associated even if relocation failed
					arr.excluded = Arrival::NotExcluded;
					SEISCOMP_DEBUG("S phase %s kept associated despite relocation failure", pick->label.c_str());
				}
				else {
...
// associate all matching picks (P, S, and depth phases)
{
	std::set<std::string> have;
	for (auto& arr : origin->arrivals) {
		// Include ALL arrivals (even excluded) to prevent duplicates
		// when _enhanceScore temporarily excludes phases
	...
	for (auto& item: pickPool) {
		...
		// Only reject picks for missing amplitude/SNR if amplitudes are required
		if ( _config.requireAmplitudes ) {
		...
		// Helper to check if association actually succeeded (not excluded)
		auto associationSucceeded = [&](const Pick* p) -> bool {
			int idx = origin->findArrival(p);
			if (idx == -1) return false;
			auto excl = origin->arrivals[idx].excluded;
			return (excl == Arrival::NotExcluded || excl == Arrival::TemporarilyExcluded);
		};

		// Determine pick type from label/phase hint
		bool pickIsS = (pick->label.find("-A-S-") != std::string::npos ||
		                pick->label.find("-S-") != std::string::npos);
		if (!pick->phase.empty()) {
			std::string ph = pick->phase;
			if (ph == "S" || ph == "Sg" || ph == "Sn" || ph == "Sb" ||
			    ph == "SKS" || ph == "ScS") {
				pickIsS = true;
			}
		}

		// For S-phase picks, try S first to avoid misidentification as excluded P
		if (pickIsS && _config.useSPhases) {
			if ( !have.count(staKey+"S") && !have.count(staKey+"SKS") && !have.count(staKey+"ScS") ) {
				if ( _associate(origin, pick, "S") || _associate(origin, pick, "SKS") || _associate(origin, pick, "ScS") ) {
					if (associationSucceeded(pick)) {
						picksAdded++;
						continue;
					}
					// If excluded, fall through to try P as fallback
				}
			}
		}

		// --- Try P phases (if station doesn't already have P/PKP) ---
		if ( !have.count(staKey+"P") && !have.count(staKey+"PKP") ) {
			if ( _associate(origin, pick, "P") || _associate(origin, pick, "PKP") ) {
				if (associationSucceeded(pick)) {
					picksAdded++;
					continue;
				}
				// If excluded, fall through to try S (for S-phase picks) or depth phases
			}
		}

		// --- Try S phases (if enabled and station doesn't already have S/SKS) ---
		if ( _config.useSPhases ) {
			if ( !have.count(staKey+"S") && !have.count(staKey+"SKS") && !have.count(staKey+"ScS") ) {
				if ( _associate(origin, pick, "S") || _associate(origin, pick, "SKS") || _associate(origin, pick, "ScS") ) {
					if (associationSucceeded(pick)) {
						picksAdded++;
					...
					// If excluded, fall through to try depth phases
				}
			}
		}

		// --- Try depth phases (if enabled and station doesn't already have them) ---
		if ( _config.useDepthPhases ) {
			if ( !have.count(staKey+"pP") ) {
				if ( _associate(origin, pick, "pP") ) {
					if (associationSucceeded(pick)) {
						picksAdded++;
						continue;
					}
			...
			if ( !have.count(staKey+"sP") ) {
				if ( _associate(origin, pick, "sP") ) {
					if (associationSucceeded(pick)) {
						picksAdded++;
						continue;
					}
			...
			if ( !have.count(staKey+"sS") ) {
				if ( _associate(origin, pick, "sS") ) {
					if (associationSucceeded(pick)) {
						picksAdded++;
						continue;
					}
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
static bool is_S_arrival(const Arrival &arr)
{
	return (arr.phase=="S"  ||
		arr.phase=="Sn" ||
		arr.phase=="Sg" ||
...
size_t Autoloc3::_removeOutliers(Origin *origin)
{
	...
	for (ArrivalVector::iterator
		...
		// S phases get more generous maxResidualKeep threshold
		double maxResidualThreshold = _config.maxResidualKeep;
		if (is_S_arrival(arr)) {
			maxResidualThreshold *= 2.5;  // S phases get 2.5x threshold
		}

		if (arr.excluded && std::abs(arr.residual) > maxResidualThreshold) {
...
bool Autoloc3::_residualOK(const Arrival &arr, double minFactor, double maxFactor) const
{
	...
	// S phases have larger picking uncertainties, so allow larger residuals
	if ( is_S_arrival(arr) ) {
		minResidual *= 2.5;  // S phases get 2.5x tolerance
		maxResidual *= 2.5;
		SEISCOMP_DEBUG("S phase residual check: %.2f in [%.2f, %.2f]", arr.residual, minResidual, maxResidual);
	}
...
bool Autoloc3::_trimResiduals(Origin *origin)
{
	...
	while (origin->definingPhaseCount() >= _config.minPhaseCount) {
		...
		for (size_t i=0; i<arrivalCount; i++) {
			...
			// S phases get more lenient normalization (they naturally have larger residuals)
			if (is_S_arrival(arr)) {
				normalizedResidual /= 2.5;  // S phases are 2.5x more forgiving
			}
...
void Autoloc3::setConfig(const Config &config) {
	_config = config;
	_associator.setMinScoreSPhase(_config.minScoreSPhase);
	_associator.setMinScoreOtherSPhases(_config.minScoreOtherSPhases);
}
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
// Phase clustering methods (DBSCAN-based)
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

void Autoloc3::_runClustering() {
	if ( !_config.clusteringEnabled ) {
		return;
	}

	// Configure the clusterer
	PhaseCluster::Config cfg;
	cfg.epsTime = _config.clusteringEpsTime;
	cfg.epsDist = _config.clusteringEpsDist;
	cfg.minPts = _config.clusteringMinPts;
	cfg.timeWeight = _config.clusteringTimeWeight;
	cfg.distWeight = _config.clusteringDistWeight;
	cfg.usePhaseType = _config.clusteringUsePhaseType;
	cfg.useTravelTime = _config.clusteringUseTravelTime;
	cfg.maxClusterTime = _config.clusteringMaxClusterTime;
	cfg.minPhases = _config.clusteringMinPhases;
	cfg.enabled = true;

	_clusterer.setConfig(cfg);

	// Collect P-phase picks ONLY (following scanloc approach)
	// S-phases will be associated to successfully located P-clusters
	std::vector<Pick*> pPicks;
	Time cutoffTime = now() - Time(120);  // Last 2 minutes

	for ( const auto& pair : pickPool ) {
		const Pick* pick = pair.second.get();
		if ( pick->time < cutoffTime ) continue;

		// Only include P-phase picks in clustering
		// Check if this is a P-phase by examining the label and phase hint
		bool isSPhase = (pick->label.find("-A-S-") != std::string::npos ||
		                 pick->label.find("-S-") != std::string::npos);

		// Also check the phase field if available
		if (!pick->phase.empty()) {
			std::string phase = pick->phase;
			// Skip S-family phases
		...
		// Include only P-phases (or unknown phases, which are likely P)
		if (!isSPhase) {
			pPicks.push_back(const_cast<Pick*>(pick));
		}
	}

	SEISCOMP_INFO("PhaseCluster: collected %zu P-phase picks for clustering (total picks in pool: %zu)",
	              pPicks.size(), pickPool.size());

	if ( (int)pPicks.size() < _config.clusteringMinPts ) {
		SEISCOMP_DEBUG("PhaseCluster: not enough P-phase picks (%zu) for clustering",
		               pPicks.size());
		return;
	}

	// Run DBSCAN clustering on P-phase picks only
	_clusterer.cluster(pPicks);

	// Report clustering results (use the clusters computed by the last cluster() call)
	// Access internal clusters through the public interface
	SEISCOMP_DEBUG("PhaseCluster: completed P-phase clustering on %zu picks", pPicks.size());
}
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
bool Autoloc3::_tryAssociateSPicks(Origin* origin)
{
	// Try to associate S-phase picks to an existing P-phase located origin
	// This follows the scanloc approach: first locate with P-phases,
	// then associate S-phases to improve location quality

	if ( !origin || origin->arrivals.empty() ) {
		return false;
	}

	// Check if this origin already has S-phases
	size_t sCount = 0;
	for ( const auto& arr : origin->arrivals ) {
		if ( arr.excluded ) continue;
		std::string phase = arr.phase;
		if ( phase == "S" || phase == "Sg" || phase == "Sn" || phase == "Sb" ||
		     phase == "SKS" || phase == "ScS" ) {
			sCount++;
		}
	...
	// Find unassociated S-phase picks in the pool
	// S-picks are typically within a time window after the P-arrivals
	Time originTime = origin->time;
	Time maxTime = now() + Time(60);  // Look up to 60 seconds in the future
	Time minTime = originTime - Time(10);  // Allow some pre-origin picks
	...
	for ( const auto& pair : pickPool ) {
		...
		// Skip picks outside time window
		if ( pick->time < minTime || pick->time > maxTime ) continue;

		// Check if this is an S-phase pick
		bool isSPhase = (pick->label.find("-A-S-") != std::string::npos ||
		                 pick->label.find("-S-") != std::string::npos);

		// Also check the phase field
		if (!pick->phase.empty()) {
			std::string phase = pick->phase;
			if (phase == "S" || phase == "Sg" || phase == "Sn" || phase == "Sb" ||
			    phase == "SKS" || phase == "ScS") {
		...
		if (!isSPhase) continue;

		// Skip picks already associated with an origin
		if ( pick->origin() != 0 && pick->origin() != origin->id ) continue;

		// Skip picks already in this origin
		if ( origin->findArrival(pick) != -1 ) continue;

		// Try to associate this S-pick
		// Compute travel time for S-phase
		const Station* station = pick->station();
		if ( !station ) continue;
		...
		// Validate S-P time consistency when a P-phase exists at this station
		if ( !validateSPConsistency(origin, pick, station, _config.maxSPResidual) ) {
			SEISCOMP_DEBUG("S-phase pick %s failed S-P consistency check at station %s",
			               pick->id.c_str(), station->code.c_str());
			continue;
		}

		// Adaptive residual tolerance based on distance
		double maxResidual = adaptiveSResidualTolerance(delta);

		// Try S, Sg, Sn phases
		TravelTime tt;
		const char* sPhases[] = { "S", "Sg", "Sn", "Sb", nullptr };
		for ( int i = 0; sPhases[i] != nullptr; ++i ) {
			if ( travelTimeS(origin->hypocenter.lat, origin->hypocenter.lon,
			                 origin->hypocenter.dep, station->lat, station->lon, 0, delta, tt) ) {
				if ( tt.phase == sPhases[i] ||
				     (delta < 15 && (sPhases[i] == std::string("Sg") || sPhases[i] == std::string("Sn"))) ) {
					// Check if travel time matches
					double expectedArrival = originTime + Time(tt.time);
					double residual = pick->time - expectedArrival;

					if ( std::fabs(residual) < maxResidual ) {
						// Associate this S-pick
						Arrival arr(pick, sPhases[i], residual);
						arr.distance = delta;
						arr.azimuth = az;
						arr.excluded = Arrival::NotExcluded;
						origin->arrivals.push_back(arr);
						...
						SEISCOMP_DEBUG("Associated S-phase pick %s to origin %ld (phase=%s, residual=%.2f, dist=%.1f, maxRes=%.1f)",
						               pick->id.c_str(), origin->id, sPhases[i], residual, delta, maxResidual);

						// Mark pick as associated
						const_cast<Pick*>(pick)->setOrigin(origin->id);
						break;
					}
				}
			}
	...
	if ( associatedCount > 0 ) {
		SEISCOMP_INFO("Associated %d S-phase picks to origin %ld", associatedCount, origin->id);

		// Re-locate with the additional S-phases
		OriginPtr relo = _relocator.relocate(origin);
		if ( relo ) {
			origin->updateFrom(relo.get());
			_updateScore(origin);
			SEISCOMP_INFO("Re-located origin %ld with S-phases: lat=%.2f, lon=%.2f, dep=%.1f, rms=%.2f, nphases=%zu",
			              origin->id, origin->hypocenter.lat, origin->hypocenter.lon,
			              origin->hypocenter.dep, origin->rms(), origin->definingPhaseCount());
			return true;
		}
		else {
			SEISCOMP_WARNING("Re-location failed after S-phase association for origin %ld", origin->id);
			// Remove the associated picks since relocation failed
			// (Keep the original P-phase location)
			return false;
		}
	}
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
bool Autoloc3::_tryAssociateAdditionalPicks(Origin* origin)
{
	// Try to associate additional picks (P, S, and depth phases) to an existing origin.
	// Improvements over basic version:
	//   * Adaptive S-phase residual tolerance based on distance
	//   * S-P time consistency validation for S picks
	//   * Automatic depth-phase (pP, sP, sS) detection for improved depth resolution
	...
	Time originTime = origin->time;
	double maxTravelTimeP = 300.0;  // Maximum P travel time (seconds)
	double maxTravelTimeS = 500.0;  // Maximum S travel time (seconds)
	double maxTravelTimeDepth = 350.0; // Depth phases arrive shortly after P
	Time maxPickTime = originTime + Time(std::max(maxTravelTimeP, std::max(maxTravelTimeS, maxTravelTimeDepth)));
	Time minPickTime = originTime - Time(10);
	...
	for ( const auto& pair : pickPool ) {
		...
		if ( pick->time < minPickTime || pick->time > maxPickTime ) continue;
		if ( pick->origin() != 0 && pick->origin() != origin->id ) continue;

		// Skip picks already in this origin (including re-fed picks with new IDs)
		if ( origin->findArrival(pick) != -1 ) continue;
		...
		double delta = 0, az = 0, baz = 0;
		delazi(&origin->hypocenter, station, delta, az, baz);
		if ( delta > _config.maxStaDist ) continue;

		// Determine pick type from label / phase hint
		bool isSPick = (pick->label.find("-A-S-") != std::string::npos ||
		                pick->label.find("-S-") != std::string::npos);
		bool isDepthPick = false;
		if (!pick->phase.empty()) {
			std::string phase = pick->phase;
			if (phase == "S" || phase == "Sg" || phase == "Sn" || phase == "Sb" ||
			    phase == "SKS" || phase == "ScS") {
			...
			if (phase == "pP" || phase == "sP" || phase == "sS" || phase == "pwP") {
				isDepthPick = true;
			}
		}
		...
		if ( isSPick && _config.useSPhases ) {
			if ( travelTimeS(origin->hypocenter.lat, origin->hypocenter.lon,
				...
				if ( std::fabs(residual) < maxResidual ) {
					// Extra validation: S-P consistency when P exists at same station
					if ( validateSPConsistency(origin, pick, station, _config.maxSPResidual) ) {
						Arrival arr(pick, tt.phase, residual);
						arr.distance = delta;
						arr.azimuth = az;
						arr.excluded = Arrival::NotExcluded;
						origin->arrivals.push_back(arr);
						...
						SEISCOMP_DEBUG("Associated additional S-pick %s to origin %ld (phase=%s, residual=%.2f, maxRes=%.1f)",
						               pick->id.c_str(), origin->id, tt.phase.c_str(), residual, maxResidual);
		...
		// --- Depth-phase association ---
		else if ( isDepthPick && _config.useDepthPhases ) {
			Seiscomp::TravelTimeList *ttlist = nullptr;
			try {
				ttlist = ttt.compute(origin->hypocenter.lat, origin->hypocenter.lon,
				                     std::max(origin->hypocenter.dep, 0.01),
				                     station->lat, station->lon, 0);
			}
			catch ( std::out_of_range & ) {
			...
			if ( ttlist ) {
				std::string matchedPhase;
				double bestResidual = 1e9;
				for ( const auto& t : *ttlist ) {
					if ( t.phase == "pP" || t.phase == "sP" || t.phase == "sS" || t.phase == "pwP" ) {
						double expected = originTime + Time(t.time);
						double residual = pick->time - expected;
						// Tight tolerance for depth phases: 3 s (they are very diagnostic)
						if ( std::fabs(residual) < 3.0 && std::fabs(residual) < std::fabs(bestResidual) ) {
							bestResidual = residual;
							matchedPhase = t.phase;
						}
					}
				}
				delete ttlist;
				if ( !matchedPhase.empty() ) {
					Arrival arr(pick, matchedPhase, bestResidual);
					arr.distance = delta;
					arr.azimuth = az;
					arr.excluded = Arrival::NotExcluded;
					origin->arrivals.push_back(arr);
					...
					SEISCOMP_INFO("Associated depth-phase pick %s to origin %ld (phase=%s, residual=%.2f)",
					              pick->id.c_str(), origin->id, matchedPhase.c_str(), bestResidual);

					const_cast<Pick*>(pick)->setOrigin(origin->id);
					// Flag origin as depth-phase constrained
					origin->depthType = Origin::DepthPhases;
				}
			}
		}
		// --- P-phase association ---
		else if ( !isSPick && !isDepthPick ) {
			if ( travelTimeP(origin->hypocenter.lat, origin->hypocenter.lon,
			                 origin->hypocenter.dep, station->lat, station->lon, 0, delta, tt) ) {
				double expectedArrival = originTime + Time(tt.time);
				double residual = pick->time - expectedArrival;
				double maxResidual = 5.0;
				...
				if ( std::fabs(residual) < maxResidual ) {
					...
					SEISCOMP_DEBUG("Associated additional P-pick %s to origin %ld (phase=%s, residual=%.2f)",
					               pick->id.c_str(), origin->id, tt.phase.c_str(), residual);
	...
	if ( associatedCount > 0 ) {
		SEISCOMP_INFO("Associated %d additional picks to origin %ld (total arrivals: %zu)",
		              associatedCount, origin->id, origin->arrivals.size());
		return true;
	}
