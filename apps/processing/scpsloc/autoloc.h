...
#ifndef SEISCOMP_PSLOC_PSLOC3
#define SEISCOMP_PSLOC_PSLOC3
...
class Autoloc3 {
	...
	public:
		...
		public:
			...
			// NEW: P/S phase locator configuration
			// Enable S phase usage in location
			bool useSPhases{true};
			
			// Minimum S/P phase ratio for quality control (0.0 = no requirement)
			double minSPRatio{0.0};
			
			// Weight multiplier for S phases (1.0 = same as P, >1.0 = higher weight)
			double sPhaseWeight{1.2};
			
			// Use depth phases (pP, sP, sS) for improved depth resolution
			bool useDepthPhases{true};
			
			// Minimum S-P time residual for association (seconds)
			double maxSPResidual{5.0};

			// Minimum origin score for S phase association (default: 5.0)
			double minScoreSPhase{5.0};

			// Minimum origin score for other S phases (ScS, SKS, etc.) association (default: 6.0)
			double minScoreOtherSPhases{6.0};

			// Require amplitudes for automatic picks to be processed
			// Set to false if using pickers that don't produce amplitudes (e.g., AI pickers)
			bool requireAmplitudes{true};
			...
			// EXPERIMENTAL!!!
			NetworkType networkType{PSLoc::GlobalNetwork};
			...
			// ============================================================
			// Phase clustering (DBSCAN-based, similar to scanloc)
			// ============================================================

			// Enable phase clustering for pick selection
			bool clusteringEnabled{false};

			// Time epsilon for clustering (seconds)
			// Picks within this time window may belong to the same cluster
			// Should be large enough to account for travel time differences
			// across the network (typically 10-60 seconds for regional events)
			double clusteringEpsTime{30.0};

			// Distance epsilon for clustering (km)
			// Picks from stations within this distance may cluster together
			double clusteringEpsDist{500.0};

			// Minimum number of picks to form a cluster
			int clusteringMinPts{3};

			// Weight for time dimension in combined distance metric
			double clusteringTimeWeight{1.0};

			// Weight for spatial dimension in combined distance metric
			double clusteringDistWeight{1.0};

			// Require compatible phase types within a cluster
			bool clusteringUsePhaseType{true};

			// Use travel-time corrected time for clustering
			bool clusteringUseTravelTime{true};

			// Maximum time span within a cluster (seconds)
			double clusteringMaxClusterTime{60.0};

			// Minimum phases for a valid cluster
			size_t clusteringMinPhases{4};

			// Use clustering to filter picks before nucleation
			// If true, only picks belonging to valid clusters are used
			bool clusteringFilterPicks{true};

			// Boost score for picks in high-quality clusters
			bool clusteringBoostScore{true};

			// Cluster score threshold for boosting (0-100)
			double clusteringScoreThreshold{10.0};

			// Score multiplier for clustered picks
			double clusteringScoreMultiplier{1.5};
...
//		bool _improve(Origin *);
		...
		// Phase clustering methods
		// Run DBSCAN clustering on P-phase picks in the pool (scanloc approach)
		void _runClustering();

		// Try to associate S-phase picks to an existing origin
		// This follows the scanloc approach: first locate with P-phases,
		// then associate S-phases to improve location quality
		bool _tryAssociateSPicks(Origin* origin);

		// Try to associate additional picks (P or S) to an existing origin
		// This is called after nucleation to improve the origin with buffered picks
		// Similar to scanloc's associate-phases-then-relocate approach
		bool _tryAssociateAdditionalPicks(Origin* origin);

		// Improvement 2: Merge duplicate origins that are close in space and time
		// This reduces false event count and consolidates picks
		void _mergeDuplicateOrigins();
