...
//	SEISCOMP_INFO("useImportedOrigins               %s",     useImportedOrigins ? "true":"false");
	...
	if ( clusteringEnabled ) {
		SEISCOMP_INFO("  Phase clustering is enabled");
		SEISCOMP_INFO("  clustering.epsTime                %.1f s",  clusteringEpsTime);
		SEISCOMP_INFO("  clustering.epsDist                %.1f km", clusteringEpsDist);
		SEISCOMP_INFO("  clustering.minPts                 %d",     clusteringMinPts);
		SEISCOMP_INFO("  clustering.minPhases              %zu",    clusteringMinPhases);
		SEISCOMP_INFO("  clustering.timeWeight             %.1f",   clusteringTimeWeight);
		SEISCOMP_INFO("  clustering.distWeight             %.1f",   clusteringDistWeight);
		SEISCOMP_INFO("  clustering.usePhaseType           %s",     clusteringUsePhaseType ? "true" : "false");
		SEISCOMP_INFO("  clustering.filterPicks            %s",     clusteringFilterPicks ? "true" : "false");
		SEISCOMP_INFO("  clustering.boostScore             %s",     clusteringBoostScore ? "true" : "false");
		if ( clusteringBoostScore ) {
			SEISCOMP_INFO("  clustering.scoreThreshold           %.1f",   clusteringScoreThreshold);
			SEISCOMP_INFO("  clustering.scoreMultiplier          %.1f",   clusteringScoreMultiplier);
		}
	}
}
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
