...
int Origin::findArrival(const Pick *pick) const
{
	...
	for (int i=0; i<arrivalCount; i++) {
		if (arrivals[i].pick == pick)
			return i;
		// Fallback: same pick ID but different pointer (re-fed pick with amplitude)
		if (arrivals[i].pick->id == pick->id)
			return i;
		// Fallback: same label but different ID (scmultipick re-feed creates new ID)
		if (arrivals[i].pick->label == pick->label)
			return i;
	}
...
void Origin::updateFrom(const Origin *other)
{
	...
	// Preserve S phases and regional phases from the original origin
	// The relocated origin (other) may only have P phases from LOCSAT
	// We need to keep phases that were added during association
	std::vector<Arrival> phasesToPreserve;
	
	for (const auto& arr : arrivals) {
		// Preserve S phases, regional phases, and depth phases
		bool shouldPreserve = isSPhase(arr.phase) || 
		                     isRegionalPhase(arr.phase) || 
		                     isDepthPhase(arr.phase);
		
		if (shouldPreserve) {
			// Check if this phase is already in the other origin
			// Use label comparison as fallback for re-fed picks with new IDs
			bool found = false;
			for (const auto& otherArr : other->arrivals) {
				if (otherArr.phase == arr.phase &&
				    (otherArr.pick->id == arr.pick->id ||
				     otherArr.pick->label == arr.pick->label)) {
					found = true;
					break;
				}
			}
			...
			if (!found) {
				phasesToPreserve.push_back(arr);
			}
		}
	}
	
	// Explicitly copy fields instead of using deprecated assignment operator
	hypocenter = other->hypocenter;
	time = other->time;
	...
	score = other->score;
	scorigin = other->scorigin;
	
	// Add back the preserved phases
	for (const auto& preservedArr : phasesToPreserve) {
		arrivals.push_back(preservedArr);
	}
