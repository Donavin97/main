...
#ifndef _SEISCOMP_PSLOC_DATAMODEL_
#define _SEISCOMP_PSLOC_DATAMODEL_
#include <string>
#include <map>
#include <vector>
...
class Pick : public Seiscomp::Core::BaseObject
{
	public:
   	...
   	public:
		...
		std::string phase;	// Phase hint from picker (P, Pg, Pn, S, Sg, Sn, etc.)
		...
		OriginID origin() const { return _originID; }
...
class OriginQuality
{
	public:
		OriginQuality() : aziGapPrimary(360), aziGapSecondary(360), 
		                  pPhaseCount(0), sPhaseCount(0), spRatio(0.0) {}

		double aziGapPrimary;
		double aziGapSecondary;
		
		// NEW: P/S phase statistics for advanced locator
		size_t pPhaseCount;    // Number of P phases used
		size_t sPhaseCount;    // Number of S phases used
		double spRatio;        // S/P phase ratio

		// Quality flag based on S/P ratio
		std::string spRatioQualityFlag() const {
