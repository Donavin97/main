...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
void MySensorLocationDelegate::setStation(const PSLoc::Station *station) {
...
bool Locator::init()
{
	...
	// Increase max iterations for better convergence (default is 100)
	// This helps LocSAT converge for difficult initial guesses
	_sclocator->setParameter("MAX_ITERATIONS", "500");
	SEISCOMP_DEBUG("Set LOCSAT MAX_ITERATIONS to 500 (current: %s)",
	               _sclocator->parameter("MAX_ITERATIONS").c_str());
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
void Locator::setStation(const PSLoc::Station *station) {
...
*/
	...
	if ( ! relo) {
		// First attempt failed - try with fixed depth as fallback
		SEISCOMP_DEBUG("Free depth relocation failed, trying fixed depth fallback");
		Origin* original = const_cast<Origin*>(origin);
		double origDepth = original->hypocenter.dep;
		original->hypocenter.dep = _minDepth;
		original->depthType = Origin::DepthManuallyFixed;
		setFixedDepth(_minDepth, true);
		
		relo = _screlocate(origin);
		
		// Restore original depth
		original->hypocenter.dep = origDepth;
		original->depthType = origin->depthType;
		useFixedDepth(false);
		
		if ( !relo ) {
			SEISCOMP_DEBUG("Fixed depth fallback also failed");
			return nullptr;
		}
	...
	// NEW: Calculate P/S phase statistics
	calculatePhaseStatistics(relo, &q.pPhaseCount, &q.sPhaseCount, &q.spRatio);
...
*/
	...
	try {
		...
		if ( !temp ) {
			SEISCOMP_WARNING("First LocSAT relocation returned nullptr, trying again");
			// Try one more time with increased iterations
			temp = _sclocator->relocate(scorigin.get());
			if ( !temp ) {
				SEISCOMP_WARNING("Second LocSAT relocation also returned nullptr");
				return nullptr;
			}
		}
		screlo = _sclocator->relocate(temp.get());
		if ( !screlo ) {
			SEISCOMP_WARNING("Second LocSAT relocation returned nullptr, returning first result");
			screlo = temp;  // Use the first result even if imperfect
		}
	}
	catch ( Seiscomp::Seismology::LocatorException& e ) {
		SEISCOMP_WARNING("LocatorException during relocation: %s", e.what());
		// Try one more time with a fresh call
		try {
			screlo = _sclocator->relocate(scorigin.get());
			if ( screlo ) {
				SEISCOMP_WARNING("Retry after exception succeeded");
			}
			else {
				SEISCOMP_WARNING("Retry also failed");
				return nullptr;
			}
		}
		catch ( Seiscomp::Seismology::LocatorException& e2 ) {
			SEISCOMP_WARNING("Second LocatorException: %s", e2.what());
			return nullptr;
		}
	}
	catch ( Seiscomp::Seismology::PickNotFoundException& e ) {
		SEISCOMP_WARNING("PickNotFoundException during relocation: %s", e.what());
		return nullptr;
	}
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
// NEW: Calculate P/S phase statistics for advanced locator
void calculatePhaseStatistics(const Origin *origin, size_t *pCount, size_t *sCount, double *spRatio)
{
	*pCount = *sCount = 0;
	...
	for (size_t i=0; i<arrivalCount; i++) {
		...
		// Count P phases (including PKP, PcP, etc.)
		if (arr.phase == "P" || arr.phase.substr(0,2) == "PK" || 
		    arr.phase.substr(0,2) == "Pc" || arr.phase == "PP") {
			(*pCount)++;
		}
		// Count S phases (including SKS, ScS, etc.)
		else if (arr.phase == "S" || arr.phase.substr(0,2) == "SK" || 
		         arr.phase.substr(0,2) == "Sc") {
			(*sCount)++;
		}
	}
	...
	// Calculate S/P ratio
	if (*pCount > 0)
		*spRatio = (double)(*sCount) / (double)(*pCount);
	else
		*spRatio = 0.0;
}
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
