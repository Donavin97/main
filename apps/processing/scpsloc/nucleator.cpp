...
// vvvvvvvvvvvvv Iteration
	...
	for (size_t i=0; i<group.size(); i++) {
		...
		// Use pick's phase hint if available, otherwise fall back to time-based heuristic
		if (!pick->phase.empty()) {
			arr.phase = pick->phase;
		}
		else {
...
double originScore(const Origin *origin, double maxRMS, double networkSizeKm)
{
	...
	size_t totalArrivals = arrivalCount;  // Include all arrivals (even excluded ones)

	// Improvement 3: Give additional credit for origins with accumulated picks
	// This helps preliminary origins that have many associated picks
	double accumulationBonus = 1.0;
	if (origin->preliminary && totalArrivals > 1) {
		// Up to 2x bonus for origins that have accumulated many picks
		// This encourages the system to keep improving preliminary origins
		accumulationBonus = std::min(2.0, 1.0 + 0.2 * std::log2((double)totalArrivals));
	}

	//int n = origin->definingPhaseCount();
	for ( size_t i=0; i<arrivalCount; i++) {
		double phaseScore = 1; // Default for P
		...
		double q = 0.8;
		if (normamp <= 0) {
			// Picks without amplitudes (e.g., from AI pickers) get default scores
			// This allows location to proceed even without amplitude information
			double amplScore = 1.0;  // Default amplitude score
			...
			switch (arr.excluded) {
				case Arrival::UnusedPhase:
					if (arr.phase.substr(0,3) == "PKP")
						phaseScore = 0.3;
					else if (arr.phase.substr(0,3) == "SKS")
						phaseScore = 0.5;
					else if (arr.phase[0] == 'S')
						phaseScore = 0.4;
					else
						phaseScore = 0.1;
					break;
				...
				case Arrival::NotExcluded:
					if (arr.phase == "P")
						phaseScore = 1.0;
					else if (arr.phase.substr(0,3) == "PKP")
						phaseScore = 0.7;
					else if (arr.phase == "S" || arr.phase.substr(0,3) == "SKS")
						phaseScore = 1.2;
					else if (arr.phase[0] == 'S')
						phaseScore = 0.9;
					else if (arr.phase == "pP" || arr.phase == "sP" || arr.phase == "sS")
						phaseScore = 1.5;
					else
						phaseScore = 0.5;
					break;
...
//		weight += amplScore;
		...
		switch (arr.excluded) {
			case Arrival::UnusedPhase:
				// Updated phase scoring for all phase types
				if (arr.phase.substr(0,3) == "PKP")
					phaseScore = 0.3;
				else if (arr.phase.substr(0,3) == "SKS")
					phaseScore = 0.5;
				else if (isSPhase(arr.phase))
					phaseScore = 0.4;
				else if (isRegionalPhase(arr.phase))
					phaseScore = 0.6;  // Regional phases slightly better than unused
				else
					phaseScore = 0.1;
				break;

			case Arrival::NotExcluded:
				// Phase-specific scoring for all phase types
				if (arr.phase == "P" || arr.phase == "Pg" || arr.phase == "Pn" || arr.phase == "Pb")
					phaseScore = 1.0;
				else if (arr.phase.substr(0,3) == "PKP")
					phaseScore = 0.7;
				else if (arr.phase == "S" || arr.phase == "Sg" || arr.phase == "Sn" || arr.phase == "Sb")
					phaseScore = 1.2;  // S phases get higher weight
				else if (arr.phase.substr(0,3) == "SKS")
					phaseScore = 1.0;
				else if (isDepthPhase(arr.phase))
					phaseScore = 1.5;  // Depth phases very valuable
				else if (isRegionalPhase(arr.phase))
					phaseScore = 0.8;  // Regional phases useful
				else
					phaseScore = 0.5;
				break;
...
//	score -= 0.1*(origin->medianStationDistance() - 30);
	...
	// Apply accumulation bonus for preliminary origins with many picks
	score *= accumulationBonus;
...
*/
	...
	// Fallback: If grid search found no candidates, create a preliminary
	// origin from the current pick's station location.
	// This follows scanloc's approach of creating preliminary cluster origins
	// even with single-station picks, which can later be improved by
	// associating more picks.
	if ( _newOrigins.empty() && pick->station() ) {
		SEISCOMP_DEBUG("GridSearch: no candidates found, creating preliminary origin from station %s",
		               (pick->net + "." + pick->sta).c_str());

		// Create origin at station location with default depth
		OriginPtr prelim = new Origin(pick->station()->lat, pick->station()->lon,
		                              2.0,  // default depth
		                              pick->time);
		prelim->depthType = Origin::DepthDefault;
		prelim->preliminary = true;

		// Add the pick as an arrival
		Arrival arr(pick, "P", 0.0);  // Assume P phase with zero residual
		arr.distance = 0.0;
		arr.excluded = Arrival::NotExcluded;
		prelim->arrivals.push_back(arr);

		_newOrigins.push_back(prelim);
		SEISCOMP_DEBUG("GridSearch: created preliminary origin with 1 arrival at station location");
	}
