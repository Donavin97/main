...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
// NEW: S-phase travel time calculation
bool travelTimeS(double lat1, double lon1, double dep1, double lat2, double lon2, double alt2, double delta, TravelTime &result)
{
	...
	for (auto& tt : *ttlist) {
		if (delta < 114) {
			// For distances < 114, take first S arrival including regional phases
			...
			// For distances >= 114, take SKS phases
	...
	delete ttlist;
	return false;  // S phase not found
}
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
// Phase type validation utilities
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

PhaseFamily getPhaseFamily(const std::string& phase)
{
	if (phase.empty()) return PhaseFamily::UNKNOWN;
	
	// P family (including regional)
	if (phase == "P" || phase == "P1" || phase == "Pg" || phase == "Pb" || 
	    phase == "Pn" || phase == "PgP" || phase.substr(0,2) == "pP" ||
	    phase.substr(0,3) == "pwP") {
		return PhaseFamily::P_FAMILY;
	}
	
	// S family (including regional)
	if (phase == "S" || phase == "S1" || phase == "S2" || phase == "Sg" || 
	    phase == "Sb" || phase == "Sn" || phase.substr(0,2) == "sS" ||
	    phase.substr(0,3) == "swS") {
		return PhaseFamily::S_FAMILY;
	}
	
	// PKP family
	if (phase.substr(0,3) == "PKP" || phase.substr(0,4) == "PKiK" || 
	    phase.substr(0,4) == "PKKP") {
		return PhaseFamily::PKP_FAMILY;
	}
	
	// SKS family
	if (phase.substr(0,3) == "SKS" || phase.substr(0,4) == "SKKS") {
		return PhaseFamily::SKS_FAMILY;
	}
	
	// Depth phases
	if (phase == "pP" || phase == "sP" || phase == "sS" || 
	    phase == "pwP" || phase == "swP" || phase == "pS" || phase == "sS") {
		return PhaseFamily::DEPTH_PHASES;
	}
	
	// Reflections
	if (phase == "PcP" || phase == "ScP" || phase == "ScS" || 
	    phase == "PcS" || phase == "ScP") {
		return PhaseFamily::REFLECTIONS;
	}
	
	// Multiples
	if (phase == "PP" || phase == "SS" || phase == "PPP" || phase == "SSS" ||
	    phase == "PcPP" || phase == "ScSS") {
		return PhaseFamily::MULTIPLES;
	}
	
	return PhaseFamily::UNKNOWN;
}

bool isValidPhase(const std::string& phase)
{
	return getPhaseFamily(phase) != PhaseFamily::UNKNOWN;
}

bool isPPhase(const std::string& phase)
{
	PhaseFamily fam = getPhaseFamily(phase);
	return (fam == PhaseFamily::P_FAMILY || fam == PhaseFamily::PKP_FAMILY);
}

bool isSPhase(const std::string& phase)
{
	PhaseFamily fam = getPhaseFamily(phase);
	return (fam == PhaseFamily::S_FAMILY || fam == PhaseFamily::SKS_FAMILY);
}

bool isRegionalPhase(const std::string& phase)
{
	// Regional phases: Pg, Sg, Pn, Sn, PcP, ScP, ScS
	return (phase == "Pg" || phase == "Sg" || phase == "Pn" || phase == "Sn" ||
	        phase == "Pb" || phase == "Sb" || phase == "PcP" || phase == "ScP" ||
	        phase == "ScS");
}

bool isTeleseismicPhase(const std::string& phase)
{
	// Teleseismic phases travel through mantle/core
	return (phase.substr(0,3) == "PKP" || phase.substr(0,3) == "SKS" ||
	        phase.substr(0,4) == "PKKP" || phase.substr(0,4) == "SKKS" ||
	        phase.substr(0,4) == "PKiK");
}

bool isDepthPhase(const std::string& phase)
{
	// Depth phases help constrain focal depth
	return (phase == "pP" || phase == "sP" || phase == "sS" ||
	        phase == "pwP" || phase == "swP" || phase == "pS");
}

std::string getPhaseCategory(const std::string& phase)
{
	switch (getPhaseFamily(phase)) {
		case PhaseFamily::P_FAMILY:      return "P";
		case PhaseFamily::S_FAMILY:      return "S";
		case PhaseFamily::PKP_FAMILY:    return "PKP";
		case PhaseFamily::SKS_FAMILY:    return "SKS";
		case PhaseFamily::DEPTH_PHASES:  return "Depth";
		case PhaseFamily::REFLECTIONS:   return "Reflection";
		case PhaseFamily::MULTIPLES:     return "Multiple";
...
std::string printOrigin(const Origin *origin, bool oneliner)
{
	...
	if (oneliner) {
		...
		if (pUsed > 0)
			out << "  S/P Ratio: " << (double)sUsed / (double)pUsed << std::endl;
		else
			out << "  S/P Ratio: N/A" << std::endl;
