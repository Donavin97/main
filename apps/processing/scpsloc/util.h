...
enum class PhaseFamily {
	P_FAMILY,       // P, Pg, Pn, Pb
	S_FAMILY,       // S, Sg, Sn, Sb
	PKP_FAMILY,     // PKP, PKPdf, PKPab, PKiKP, PKKP
	SKS_FAMILY,     // SKS, SKKS, SKKSdf
	DEPTH_PHASES,   // pP, sP, sS, pwP, swP
	REFLECTIONS,    // PcP, ScP, ScS
	MULTIPLES,      // PP, SS, PPP, SSS
	UNKNOWN
};
...
bool isRegionalPhase(const std::string& phase);  // Pg, Sg, Pn, Sn
bool isTeleseismicPhase(const std::string& phase);  // PKP, SKS, etc.
bool isDepthPhase(const std::string& phase);  // pP, sP, sS
std::string getPhaseCategory(const std::string& phase);  // Returns category name for logging
